...
namespace mt {
	...
	protected:
		HANDLE	m_hHandle;
	public:
		event(LPCTSTR name, bool Inherit, long Access)
		{
			m_hHandle = ::OpenEvent( Access, Inherit, name);
		}
		event(bool ManualReset, bool State, LPCTSTR name,securityattributes* pSA = NULL)
		{
	...
	protected:
		HANDLE		m_hHandle;
	public:
		mutex(bool LockObject = false,LPCTSTR name = NULL,securityattributes* pSA = NULL)
		{
			m_hHandle = ::CreateMutex(pSA,LockObject,name);
		}
		mutex(LPCTSTR name,bool Inherit = false,long Access = MUTEX_ALL_ACCESS)
		{
	...
	protected:
		...
		HANDLE      m_hfm;
	public:
		optex(LPCTSTR name = NULL,long nSpinCount = 4000)
		{
			TCHAR szResult[100]={0};
			wsprintf(szResult,_T("OPTEX_E_%s"),name);	m_hevt = CreateEvent(NULL, FALSE, FALSE, szResult);
			wsprintf(szResult,_T("OPTEX_M_%s"),name);	
